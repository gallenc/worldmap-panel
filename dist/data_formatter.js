    ...
    execute: function () {
      ...
      DataFormatter = function () {
        ...
        _createClass(DataFormatter, [{
          ...
          key: 'setOpenNMSValues',
          value: function setOpenNMSValues(dataList, data) {
            if (dataList && dataList.length > 0) {
              var highestValue = 0;
              var lowestValue = Number.MAX_VALUE;
              ...
              var dataNames = [];
              for (var _x = 0; _x < dataList.length; _x += 1) {
                if (dataList[_x].target) {
                  var pos = dataList[_x].target.lastIndexOf('.latitude');
                  if (pos > 0) {
                    var name = dataList[_x].target.substring(0, pos);
                    dataNames.push(name);
                  }
                }
              }
              // fill in the values for the datapoint names
              for (var _i = 0; _i < dataNames.length; _i += 1) {
                var latvar = void 0;
                var lonvar = void 0;
                var valvar = void 0;
                for (var _y = 0; _y < dataList.length; _y += 1) {
                  if (dataList[_y].target && dataList[_y].target.includes(dataNames[_i])) {
                    var datapoints = dataList[_y].datapoints;
                    var dp = void 0;
                    var num = void 0;
                    if (datapoints && datapoints.length !== 0) {
                      var _z = datapoints.length - 1;
                ...
                if (latvar && lonvar && valvar) {
                  // if we have all 3 numbers create a datapoint
                  var dataValue = {
                    key: dataNames[_i],
